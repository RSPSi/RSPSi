...
public class FloorDefinitionLoaderOSRS extends FloorDefinitionLoader {
	...
	public void init(Archive archive) {
		...
		try {
			ByteBuffer buffer = ByteBuffer.wrap(Files.readAllBytes(new File(Config.cacheLocation.get() + "flo.dat").toPath()));
			...
			buffer = ByteBuffer.wrap(Files.readAllBytes(new File(Config.cacheLocation.get() + "flo2.dat").toPath()));
