...
public class MyObjectDefinitionLoader extends ObjectDefinitionLoader {
	...
	private Buffer data, data667;
	
	private int[] indices, indices667;
	...
	public void init(Archive config) {
		try {
			data667 = new Buffer(Files.readAllBytes(new File(Config.cacheLocation.get() + "667loc.dat").toPath()));
			Buffer idx667 = new Buffer(Files.readAllBytes(new File(Config.cacheLocation.get() + "667loc.idx").toPath()));
			count = idx667.readUShort();
			indices667 = new int[count];
			int offset = 2;
			for (int index = 0; index < count; index++) {
				indices667[index] = offset;
				offset += idx667.readUShort();
			}
			System.out.println("Expected " + count + " 667 ids");
			
			data = new Buffer(Files.readAllBytes(new File(Config.cacheLocation.get() + "obj.dat").toPath()));
			Buffer buffer = new Buffer(Files.readAllBytes(new File(Config.cacheLocation.get() + "obj.idx").toPath()));
			int count525 = buffer.readUShort();

			System.out.println("Expected " + count525 + " 525 ids");
			indices = new int[count525];
			offset = 2;
			for (int index = 0; index < count525; index++) {
	...
	public ObjectDefinition forId(int id) {
		...
		if(id >= indices.length) {
			...
			data667.setPosition(indices667[id]);
			definition = decode667(id, data667);
		} else {
