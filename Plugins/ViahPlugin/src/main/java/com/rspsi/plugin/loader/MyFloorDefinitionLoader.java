...
public class MyFloorDefinitionLoader extends FloorDefinitionLoader {
	...
	private Floor[] overlays, underlays, overlaysOSRS, underlaysOSRS;
	...
	public void init(Archive archive) {
		...
		ByteBuffer osrsBuffer = ByteBuffer.wrap(archive.file("flo3.dat").getData());
		underlaysOSRS = new Floor[osrsBuffer.getShort()];

		for (int i = 0; i < underlaysOSRS.length; i++) {
			underlaysOSRS[i] = decodeUnderlayOSRS(osrsBuffer);
			underlaysOSRS[i].generateHsl();
		}

		overlaysOSRS = new Floor[osrsBuffer.getShort()];

		for (int i = 0; i < overlaysOSRS.length; i++) {
			overlaysOSRS[i] = decodeOverlayOSRS(osrsBuffer);
			overlaysOSRS[i].generateHsl();
		}
	}
	...
	public Floor decodeUnderlayOSRS(ByteBuffer buffer) {
	...
	public Floor decodeOverlayOSRS(ByteBuffer buffer) {
		Floor floor = new Floor();
		while (true) {
			int opcode = buffer.get();
			if (opcode == 0) {
