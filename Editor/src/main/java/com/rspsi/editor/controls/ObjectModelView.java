...
public class ObjectModelView extends VBox {
	...
	private ModelInstance nullModel;
	...
	public ObjectModelView() {
		...
		ChangeListenerUtil.addListener(true, () -> {
			...
			try {
				...
				model.rotateClockwise();
				nullModel = model.toModelInstance(64, 768 * 5, -50, -10, -50);
	...
	public void prepareView(ObjectDataset cell) {
		...
		try {
			...
			Renderable m = def.modelAt(new int[1][1], cell.getType() == 11 ? 10 : type, 1, 0, 0, 0, 0, -1, 0);
			...
			if(m instanceof Mesh) {
				m = ((Mesh)m).toModelInstance(64, 768 * 5, -50, -10, -50);
			}
			model = (ModelInstance) m;
			sprite = null;
			renderModel();
		} catch (Exception e) {
			e.printStackTrace();
