...
import com.rspsi.editor.controllers.ObjectPreviewController;
import com.rspsi.editor.controls.ObjectModelView;
...
import com.rspsi.editor.swatches.ObjectSwatch;
...
public class ObjectPreviewWindow extends Application {
						...
						showVarbit.setOnAction(al -> {
							...
							if(optObj.isPresent()) {
								if(!optObj.get().getValue().equalsIgnoreCase("-1")) {
									...
									FXDialogs.showInformation(stage,"Varbit [" + optObj.get().getValue() + "]", "Setting: " + bit.getSetting() + "" + System.lineSeparator() + "High: " + bit.getHigh() + "" + System.lineSeparator() + "Low: " + bit.getLow());
								} else {
	...
	public void loadToSwatches(List<ObjectDataset> data) {
		ModelInstance oldModel = view.getModel();
