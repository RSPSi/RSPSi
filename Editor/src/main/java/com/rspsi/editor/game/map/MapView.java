			...
			Platform.runLater(() -> {
				...
				if(f != null) {
					try {
						...
						FXDialogs.showError(MainWindow.getSingleton().getStage().getOwner(), "Error while saving map_index", "There was a failure while attempting to save" + System.lineSeparator() + "the map_index to the selected file.");
						e.printStackTrace();
					}
				}
			});
