	...
	private static final ExecutorService executorService = Executors.newCachedThreadPool();
		...
		public int[] drawMinimapOriented(int plane) {
			...
			for (int y = 0; y < 64; y++) {
				int i1 = (y) * 4;
				for (int x = 0; x < 64; x++) {
					val pos = new Vector3i(x, y, plane);
					val posAbove = new Vector3i(x, y, plane + 1);
					if ((mapRegion.tileFlags[plane][x][y] & 0x18) == 0) {
						sceneGraph.drawMinimapTile(raster, pos, i1, 256);
					}

					if (plane < 3 && (mapRegion.tileFlags[plane + 1][x][y] & 8) != 0) {
						sceneGraph.drawMinimapTile(raster, posAbove, i1, 256);
					}
					i1 += 4;
				}
			}
