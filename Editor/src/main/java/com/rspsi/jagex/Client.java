...
import com.rspsi.editor.game.DisplayCanvas;
...
import com.rspsi.jagex.draw.font.RSFont;
...
import com.rspsi.jagex.util.TextRenderUtils;
...
import static com.rspsi.editor.MainWindow.glEditorWindow;
...
public final class Client implements Runnable {
    ...
    public RenderMode getRenderMode() {
        return Options.useGlRendering.get() ? RenderMode.GPU : RenderMode.SOFTWARE;
    }
    ...
    public ObjectProperty<LoadState> loadState = new SimpleObjectProperty<>(LoadState.CLIENT_INIT);
	...
	*/
    ...
    public final void displayErrorMessage() {
        ...
        if (getRenderMode() == RenderMode.SOFTWARE) {
            ...
            if (error) {
                ...
                context.fillText("An error has occured while booting the map editor", this.getCanvasWidth() / 2f, this.getCanvasHeight() / 2f);
                if (!errorMessage.isEmpty()) {
                    context.fillText(errorMessage, this.getCanvasWidth() / 2f, this.getCanvasHeight() / 2f + 20f);
                }
            }
            ...
            if (unableToLoad) {
                ...
                context.setTextAlign(TextAlignment.CENTER);
                context.setTextBaseline(VPos.CENTER);
                context.fillText("Error - unable to load editor!", this.getCanvasWidth() / 2f, this.getCanvasHeight() / 2f);
            }
        } else {
            if (error) {
                glEditorWindow.postRender.add(gl -> {
                    glEditorWindow.drawText("An error has occured while booting the map editor", this.getCanvasWidth() / 2, this.getCanvasHeight() / 2);
                    if (!errorMessage.isEmpty()) {
                        glEditorWindow.drawText(errorMessage, this.getCanvasWidth() / 2, this.getCanvasHeight() / 2 + 20);
                    }
                });
            }
        }
    ...
    public final void drawGameScreen() {
        ...
        if (getRenderMode() == getRenderMode().SOFTWARE) {
            if (loadState.get() == LoadState.WAITING_INPUT) {
            ...
            if (loadState.get() == LoadState.ERROR) {
            ...
            if (loadState.get() == LoadState.ACTIVE) {
                renderView();
            }
        ...
        if (loadState.get() == LoadState.ACTIVE && SceneGraph.minimapUpdate) {
        ...
        if (gameImageBufferNeedsInit && getRenderMode() == RenderMode.SOFTWARE) {
    ...
    public final void load() {
        ...
        try {
            ...
            if (cache.getIndexedFileSystem().is317()) {
                ...
                try {
                    byte[] data = cache.readFile(CacheFileType.SPRITE).archive("mapscene").file(0).getData();
    ...
    public void drawCenteredText(String text) {
        if (getRenderMode() ==RenderMode.SOFTWARE) {
            gameImageBuffer.initializeRasterizer();
            gameImageBuffer.clear(0);
            TextRenderUtils.renderCenter(gameImageBuffer.getGraphics(), text, gameCanvas.getWidth() / 2, gameCanvas.getHeight() / 2, 0xFFFFFF);
            gameImageBuffer.finalize();
            drawGameImage();
        } else {
            glEditorWindow.postRender.add(gl -> {
                glEditorWindow.drawText(text, this.getCanvasWidth() / 2, this.getCanvasHeight() / 2);
            });
        }
    ...
    public final void loadCoordinates(int wX, int wY, int chunkXLength, int chunkYLength) {
        ...
        chunks.clear();
        drawCenteredText("Loading map, this may take a few seconds...");
        ...
        loadState.set(LoadState.LOADING_MAP);
    ...
    public final void loadNextRegion() {
        try {
            if (loadState.get() == LoadState.LOADING_MAP) {
                boolean j = method54();
        ...
        if (loadState.get() == LoadState.ACTIVE && plane != anInt985) {
    ...
    public final void gameFinishedLoading() {
        loadState.set(LoadState.WAITING_INPUT);
        ...
        if (Options.showDebug.get()) {
          ...
          /*  k += TextRenderUtils.renderLeft(gameImageBuffer, "Mouse Tile: " + sceneGraph.hoveredTileX + "," + sceneGraph.hoveredTileY + "", c, k,
           ...
           /* if (sceneGraph.hoveredTileX >= 0 && sceneGraph.hoveredTileY >= 0
                    && sceneGraph.tiles[Options.currentHeight.get()][sceneGraph.hoveredTileX][sceneGraph.hoveredTileY] != null) {
...
/*		int l = xCameraPos;
        ...
        for (Consumer<SceneGraph> r : Lists.newArrayList(SceneGraph.onCycleEnd)) {
            if (r != null) {
                try {
                    r.accept(sceneGraph);
                } catch (Exception ex) {
    ...
    public final void loadChunks() {
        ...
        //ObjectDefinition.baseModels.clear();
        GLEditorWindow.shouldUploadScene = true;
    ...
    public final boolean method54() {
        ...
        loadChunks();
        loadState.set(LoadState.ACTIVE);
        return true;
    }
    ...
    public final void processLoadedResources(ResourceResponse response) {
        byte[] unzipped;
        try {
            unzipped = GZIPUtils.decompress(response.getData());
    ...
    public final void pulseGame() {
        if (loadState.get() == LoadState.CLIENT_INIT)
            return;
    ...
    public final void shutdown() {
        try {
            ...
            errorMessage = "";
            errorDisplayed.set(false);
            runLater.clear();
            SceneGraph.onCycleEnd.clear();
            method118();
            if (MeshLoader.getSingleton() != null)
