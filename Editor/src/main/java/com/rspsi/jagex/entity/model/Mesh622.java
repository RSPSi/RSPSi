...
public class Mesh622 extends Mesh {
	...
	public Mesh622(byte[] abyte0) {
        ...
        if (l2 == 1 && numTexTriangles > 0) {
            textureCoordinates = this.textureCoordinates = new byte[numTriangles];
        }
        ...
        for (int i12 = 0; i12 < numTriangles; i12++) {
            ...
            if (textureCoordinates != null)
                if (textureIds[i12] != -1)
                    textureCoordinates[i12] = this.textureCoordinates[i12] = (byte) (nc7.readUByte() - 1);
                else
                    textureCoordinates[i12] = this.textureCoordinates[i12] = -1;
        }
        nc1.setPosition(k7);
        nc2.setPosition(j6);
        int k12 = 0;
