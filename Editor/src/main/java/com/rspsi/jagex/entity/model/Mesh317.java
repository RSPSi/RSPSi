...
public class Mesh317 extends Mesh {
	...
	public Mesh317(byte[] data) {
		...
		if(useTextures == 1){
			...
			textureCoordinates = new byte[numFaces];
		}
		...
		for (int face = 0; face < numFaces; face++) {
			...
			if (useTextures == 1) {
				...
				faceTypes[face] = (type & 1) == 1 ? 1 : 0;

				if((type & 2) == 2){
					this.textureCoordinates[face] = (byte)(type >> 2);
		...
		for (int index = 0; index < numTextures; index++) {
			...
			textureMappingA[index] = maps.readUShort();
			textureMappingB[index] = maps.readUShort();
			textureMappingC[index] = maps.readUShort();
		}
		...
		if(this.textureCoordinates != null) {
			...
			for(int var43 = 0; var43 < numFaces; ++var43) {
				int var44 = this.textureCoordinates[var43] & 255;
				if(var44 != 255) {
					if(this.faceIndicesA[var43] == (this.textureMappingA[var44] & '\uffff') && this.faceIndicesB[var43] == (this.textureMappingB[var44] & '\uffff') && this.faceIndicesC[var43] == (this.textureMappingC[var44] & '\uffff')) {
						this.textureCoordinates[var43] = -1;
					} else {
						var46 = true;
					}
				}
